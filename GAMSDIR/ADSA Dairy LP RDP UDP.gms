...
Set ax xMultiSolves / MAxWH1, MAxWH2, MAxWH3, MAxWH4, MAxWH5,
                      ...
                      Prod9000s /;
$offtext
Set ax xMultiSolves / Dummy1*Dummy2 /;
...
Parameter DMIAdv(s,c);
DMIAdv(s,c)= sum(y, DMIs(s--(ord(c)-1),y) );
display DMIAdv;
...
Parameter MEMaintAdv(s,c);
MEMaintAdv(s,c)=sum(x, MEMaints(s--(ord(c)-1),x) );
display MEMaintAdv;
...
*Protein
*Table Prots(s,w)  ADPLS Protein for lactation months (kg DM)
*$ondelim
*$include Prots.csv
*$offdelim
*display Prots;

*Parameter ProtsAdv(s,c);
*ProtsAdv(s,c)=sum(w, Prots(s--(ord(c)-1),w) );
*display ProtsAdv;
...
Scalar CullRevenuePerMilker Cull and calf sales per milker      / 164 /;
...
*                 vConsToCows(o,s,c,f,h) Transfer conserved feed to cows   (DM)
                 ...
                 vMEintake(s,c)         ME intake per day for cows        (MJ)
                 vRDPTransfer(s,c)      RDP transfer to ADPLS via rumen   (kg)
                 vDMIntake(s,c)         Actual DM intake per day for cows (kg)
...
Equations  eObjFunction(s)        ObjectiveFunction -> max final cash balance       # Equation 1 (part of)
           eAreaSum               ForageArea must be less than TotalArea            # Equation 2
...
***Equation 2
eAreaSum..
  + sum(f, AF('summer',f)  * vForageArea(f) ) + vLeaseOut         =l= TotalArea;
...
*assume same digestibility as source forage
  ...
  -        sum(y, DMIAdv(c,s)       * vNumberCows(c)          )
                                                                         =l= 0;
***Equation 7
eSupplRest(s,c,r)..
  + sum(g,                                  vGrainToCows(s,c,g,r) )
  - sum(y, Substitution(r) * DMIAdv(s,c)* vNumberCows(c)        )
                                                         =l= 0;
...
*assume same digestibility as source forage
  ...
  +        sum(x, MEMaintAdv(c,s)     * vNumberCows(c)          )
...
*  +        sum(x, ProtsAdv(c,s,x)   * vNumberCows(c)          )
*                                                         =l= 0;
...
eTCWaterPurch(s,axNow)..
    vWaterReqt(s) * xMultiSolvesTable(axNow,'dWater')            =l= vTCWaterPurch(s) ;              # using axNow as dynamic sete
...
eSumCowsLimit(axNow)..
  sum(c, vNumberCows(c))                           =l= xMultiSolvesTable(axNow,'dMaxCows');              #ax as dynamic sete
...
Scalar CowUnitValue      /   700/;
Scalar RepUnitValue      /   400/;
Scalar LandUnitValue     / 10000/;

Scalar CowAppnRate       / 0.03/;
Scalar IrriAppnRate      /-0.10/;
Scalar TracAppnRate      /-0.20/;
Scalar PandEAppnRate     /-0.20/;
Scalar DairyAppnRate     /-0.06/;
Scalar ShedsAppnRate     /-0.06/;
Scalar LandAppnRate      / 0.05/;

Scalar RepRate Replacement rate / 0.25 /


Scalar IrriAssetValue     /500000/;
Scalar TracAssetValue     /200000/;
Scalar PandEAssetValue    /200000/;
Scalar DairyAssetValue    /800000/;
Scalar ShedsAssetValue    /100000/;
Scalar LandAssetValue;
LandAssetValue = LandUnitValue * TotalArea;

         Scalar CowAssetValue;
         Scalar RepAssetValue;
         Scalar TotalAssetValue;
         Scalar Appreciation;
         Scalar ProfitAfterAppn;
         Scalar ROA;

Parameter Report1(*,                      ax) Reporting on important info ;
Parameter Report2(*,                      ax) Reporting on important info ;

*Execute_Loadpoint 'C:\Documents and Settings\Neal\My Documents\gamsdir\DairyFarm_p'  # This loads a basis
...
*        Calculating asset values
         ...
         CowAssetValue = CowUnitValue* sum(s, vNumberCows.l(s) );

         RepAssetValue = RepUnitValue* sum(s, vNumberCows.l(s) )* RepRate * 2;


         TotalAssetValue = CowAssetValue  + RepAssetValue   + IrriAssetValue
                + TracAssetValue + PandEAssetValue + DairyAssetValue
                + ShedsAssetValue + LandAssetValue ;


         Appreciation =     CowAssetValue   * CowAppnRate
                 + RepAssetValue   * CowAppnRate
                 + IrriAssetValue  * IrriAppnRate
                 + TracAssetValue  * TracAppnRate
                 + PandEAssetValue * PandEAppnRate
                 + DairyAssetValue * DairyAppnRate
                 + ShedsAssetValue * ShedsAppnRate
                 + LandAssetValue  * LandAppnRate ;


         ProfitAfterAppn = vObjective.l + Appreciation ;


         ROA = ProfitAfterAppn / TotalAssetValue ;
...
*        Reporting
         ...
         Report2( "Profit before Appn",   ax) = vObjective.l          ;
         Report2( "Appreciation",         ax) = Appreciation          ;
         Report2( "Profit after Appn",    ax) = ProfitAfterAppn          ;
         Report2( "ROA",                  ax) = ROA          ;
         Report2( "Total Asset Value",    ax) = TotalAssetValue          ;
         axNow(ax) = No;
) ; # {endloop}
